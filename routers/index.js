...
const md5 = require('blueimp-md5')  // 加密密码
...
// 添加考勤信息
router.post('/manage/work/add', (req, res) => {
  const Work = req.body
...
// 获取考勤信息分页列表
router.get('/manage/work/list', (req, res) => {
  const {pageNum, pageSize} = req.query
  WorkModel.find({})
    .then(Works => {
      res.send({status: 0, data: pageFilter(Works, pageNum, pageSize)})
    })
    .catch(error => {
      console.error('获取商品列表异常', error)
      res.send({status: 1, msg: '获取商品列表异常, 请重新尝试'})
...
// 更新考勤信息
router.post('/manage/work/update', (req, res) => {
  const Works = req.body
  WorkModel.findOneAndUpdate({_id: Works._id}, Works)
    .then( Works => {
      res.send({status: 0})
    })
    .catch(error => {
